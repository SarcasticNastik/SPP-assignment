...
void floyd_warshall() {
  ...
  /* /1* #pragma omp parallel for shared(adj, vertex) private(k, i, j) *1/ */
  /* for (k = 0; k < vertex; ++k) { */
  /* #pragma omp parallel for private(j) shared(adj, vertex) */
  /*   for (i = 0; i < vertex; ++i) { */
  /*     ele1 = adj[i * vertex + k]; */
  /*     if (ele1 >= 100000) */
  /*       continue; */
  ...
  for (til_i = 0; til_i < vertex / PARAM_X + 1; til_i++)
    for (k = 0; k < PARAM_X && til_i * PARAM_X + k < vertex; k++) {
      for (i = 0; i < vertex; i++) {
        int val = adj[i * vertex + til_i * PARAM_X + k];
        for (j = 0; j < vertex; j++) {
          adj[i * vertex + j] =
              min(adj[i * vertex + j],
                  val + adj[(til_i * PARAM_X + k) * vertex + j]);
        }
      }
    }
}
